...
export default function Home() {
  ...
  return <div className="recon-site" id="top">
    ...
    <footer className="recon-footer"><h2>{content.footer.heading}</h2><EmailButton email={content.footer.email} compact /><div className="recon-footer-bottom"><div>{safeWebUrl(content.footer.githubUrl) && <a href={safeWebUrl(content.footer.githubUrl)} target="_blank" rel="noopener noreferrer">{content.footer.githubLabel}</a>}{safeWebUrl(content.footer.linkedinUrl) && <a href={safeWebUrl(content.footer.linkedinUrl)} target="_blank" rel="noopener noreferrer">{content.footer.linkedinLabel}</a>}</div><span>© {new Date().getFullYear()} {content.footer.copyrightName}</span></div></footer>
  </div>;
}
